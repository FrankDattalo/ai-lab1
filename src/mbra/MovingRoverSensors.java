...
/**
 * MovingRoverSensors: A class for reading visual and sample perceptions from a
 * file
 *
...
public class MovingRoverSensors {
	...
	private void initialize(BufferedReader myFile) {
		...
		try {
			...
			this.samps = new SamplePercept[x1.length][2];
			this.vis = new VisionPercept[x1.length][2];

			int i;
			for (i = 0; i < x1.length; i++) {
				this.samps[i][0] = (SamplePercept) x1[i];
				this.samps[i][1] = (SamplePercept) x2[i];
				this.vis[i][0] = (VisionPercept) x3[i];
				this.vis[i][1] = (VisionPercept) x4[i];
			}
	...
	public static void main(String args[]) {
		...
		MovingRoverSensors sensors = null;

		if (args.length == 0) {
			sensors = new MovingRoverSensors();
		} else {
			sensors = new MovingRoverSensors(args[1]);
		}

		ModelBasedReflexAgent agent = new ModelBasedReflexAgent(sensors);
		...
		do {
			...
			System.out.printf("Position: <%d, %d> Looking: %5s Percieved: <%2d, %s>", state.x, state.y, state.facing,
					state.percept.getSampleValue(), state.percept.isVisionIsClear() ? "CLEAR" : "BLOCKED");
